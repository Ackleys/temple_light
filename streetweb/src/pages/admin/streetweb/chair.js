...
const Chair = require('create-react-class')({
    ...
    inputChange:function(type,e){
        if(type === 'title'){
            ...
            this.state.data.value = e.target.value*60;
    ...
    render: function (){
        ...
        if(self.state.ddata.value){
            value = self.state.ddata.value/60;
        }
        ...
        return (
            <MyLayout>
                <p className='doll_title'>倒计时类型机器可选套餐
                ...
                >
                    ...
                    <Input defaultValue={value} placeholder='请输入时间（单位分钟），例如：1' onChange={self.inputChange.bind(self,'value')} />
                    {alerts}
                </Modal>
            </MyLayout>
        );
