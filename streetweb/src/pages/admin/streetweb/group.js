...
import { Button, Menu, Table, Modal, Input, Alert, message, Select, Popconfirm } from 'antd';
...
const Group = require('create-react-class')({
    ...
    editmoney:function(){
        let self = this;
        let url = window.API_PREFIX+'/device/rate/update';
        var callback = function(err,res){
            if(err){
                self.setState({errormsg:err.err.msg});
            }else{
               message.success('修改成功！');
               self.setState({input_show:false,visible:false,editmoney:false});
               self.equ_list();
            }
        };
        let data = {
            imei:self.state.self_imei, 
            rate:parseFloat(self.state.rate),
        }        
        var xhr  = new XMLHttpRequest();
        xhr.open("PATCH", url);
        xhr.setRequestHeader("Content-Type", "application/json;charset=utf-8") ; 
    ...
    input_change:function(e){
        ...
        if(e.target.value<=1&&e.target.value>=0){
            ...
            this.setState({errormsg:'请输入小于等于1大于等于零的数字'});
    ...
    recycle:function(re){
        ...
        var callback = function(err,res){
            if(err){
                ...
                self.equ_list(pager.current,pager.pageSize);
            }
        };
    ...
    render: function (){
        ...
        let columns = [{
                    ...
                    render:(text,re) => {
                        ...
                        if(self.state.level === 4){
                            ...
                            data = [{agent:'省级代理','fencheng':re.l3},{agent:'市县代理','fencheng':re.l2},{agent:'运营商','fencheng':re.l1}];
                        }else if(self.state.level === 3){
                            top = [re.l4,0] 
                            mid = re.l3;
                            bot = [re.l2,re.l1];
                            data = [{agent:'市县代理','fencheng':re.l2},{agent:'运营商','fencheng':re.l1}];
                            
                        }else if(self.state.level === 2){
                            top = [re.l4,re.l3]
                            mid = re.l2;
                            bot = [re.l1,0];
                            data = [{agent:'运营商','fencheng':re.l1}];
                        ...
                        return(
                            <span>
                                <a href="#" style={{color: '#666'}}>{(top.reduce(accAdd)*100).toFixed(2)+'%'}
                                </a>
                                <span className="ant-divider" />
                                <a href="#" onClick = {this.myself.bind(self,re.imei)}>{mid*100+'%'}</a>
                                <span className="ant-divider" />
                                <a href="#" onClick={this.bottom_agent.bind(self,data)}>{(bot.reduce(accAdd)*100).toFixed(2)+'%'}
                                </a>
                            </span>
                        )},
                }];
        if(self.state.level === 4){
            columns.push({
                    ...
                    dataIndex: 'back',
                    render:(text,re)=>{
                        return  <Popconfirm title="收回后该设备相关信息都会删除，确定要收回吗?" onConfirm={self.recycle.bind(self,re)} okText="是" cancelText="否">
                                    <Button>收回</Button>
                                </Popconfirm>
                    }
                })
        ...
        return (
          <MyLayout level={(l)=>{self.getlevel(l)}}>
