...
import { Button, Menu, Modal, Input, message, Alert, Popconfirm } from 'antd';
import {MyLayout}     from '../common/layout.js';

const Chair = require('create-react-class')({
    ...
    handleOk:function(){
        ...
        if(da.value>20){
            self.setState({errormsg:"脉冲数不可大于20"});
            return false;
        }else if(da.value%1>0){
            self.setState({errormsg:"脉冲数不可为小数"});
            return false;
        }
    ...
    inputChange:function(type,e){
        if(type === 'title'){
            ...
            this.state.data.value = e.target.value-0;
    ...
    render: function (){
        ...
        return (
            <MyLayout>
                <p className='doll_title'>投币器类型机器可选套餐
                ...
                >
                    ...
                    <Input defaultValue={price} placeholder='请输入钱数（单位元），例如：1' onChange={self.inputChange.bind(self,'price')} />
                    <Input defaultValue={value} placeholder='请输入脉冲个数，例如：1' onChange={self.inputChange.bind(self,'value')} />
                    {alerts}
                </Modal>
            </MyLayout>
        );
